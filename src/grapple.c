...
void do_bearhug(P_char ch, char *argument, int cmd)
{
  ...
  if(victim)
  {
    ...
    percent += GET_C_DEX(ch) - GET_C_AGI(victim);
...
void do_headlock(P_char ch, char *argument, int cmd)
{
  ...
  if(victim)
  {
    ...
    percent += (GET_C_STR(ch) - GET_C_STR(victim)) / 2;
    percent += GET_LEVEL(ch) - GET_LEVEL(victim);
...
void event_headlock(P_char ch, P_char victim, P_obj obj, void *data)
{
  ...
  else
  {
    ...
    percent = BOUNDED(0, (GET_C_STR(ch) - GET_C_STR(victim)), 5 + gclvl) + number(-2, 2);
...
void armlock_check(P_char attacker, P_char grappler)
{
  ...
  percent += GET_LEVEL(grappler) - GET_LEVEL(attacker);
...
void do_leglock(P_char ch, char *argument, int cmd)
{
  ...
  if(victim)
  {
    ...
    percent += GET_LEVEL(ch) - GET_LEVEL(victim);
    percent += (GET_C_DEX(ch) - GET_C_STR(victim)) / 2;
    lag = 0;
