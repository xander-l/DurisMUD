...
void do_trapset(P_char ch, char *argument, int cmd)
{
	...
	else if (!str_cmp(arg2, "charge"))
	{
		...
		{
			send_to_char("Allowed range is -1 to 100. (0 disarms, -1 unlimited)\r\n", ch);
			return;
		}
		obj->trap_charge = val;
	...
	switch (obj->trap_dam)
	{
		case TRAP_DAM_SLEEP:
			if (!IS_SET(obj->trap_eff, TRAP_EFF_ROOM))
			{
				...
				act("&+LA strange gas pours forth from $p, making you feel very sleepy ..... zzzzzz", FALSE, ch, obj, 0, TO_CHAR);
				if (GET_OPPONENT(ch))
					stop_fighting(ch);
				if (IS_DESTROYING(ch))
					stop_destroying(ch);
				...
				{
					act("&+LA strange gas pours forth from $p, and $n goes to sleep.", TRUE, ch, obj, 0, TO_ROOM);
			...
			else
			{
				LOOP_THRU_PEOPLE(wch, ch)
				{
					...
					act("&+LA strange gas pours forth from $p, making you feel very sleepy ..... zzzzzz", FALSE, wch, obj, 0, TO_CHAR);
					if (GET_OPPONENT(wch))
						stop_fighting(wch);
		...
		case TRAP_DAM_TELEPORT:
			...
			break;
		case TRAP_DAM_FIRE:
			messages.attacker       = "The &+rfireball&N hits $N.";
			messages.victim         = "You are enveloped in &+rflames from a fireball&N sent by $P - OUCH!";
			messages.room           = "The &+rfireball&N explodes into the face of $N.";
			messages.death_attacker = "The &+rfireball&N hits $N with full force, causing an immediate death.";
			messages.death_victim   = "As the &+rfireball&N hits you, you burst into &+rflames&N and die.";
			messages.death_room     = "The heat from the &+rfireball&N turns $N into a charred corpse.";
			...
			break;
		case TRAP_DAM_COLD:
			messages.attacker       = "&+BThe blast of &+ccold&+B strikes $N &+Bdead on.";
			messages.victim         = "&+BThe blast of &+ccold&+B strikes you dead on, freezing up your limbs! BRRR.";
			messages.room           = "&+BThe blast of &+ccold&+B hits $N&+B, who screams out in pain!";
			messages.death_attacker = "&+BThe blast of &+ccold&+B leaves $N &+Cfrozen &+Bsolid!";
			messages.death_victim   = "&+BYour body turns to &+Cice&+B the blast of &+ccold&+B hits you, AARRGHGHHGH!!";
			messages.death_room     = "&+BThe blast of &+ccold&+B hits $N&+B, who &+Cfreezes solid&+B and dies instantly!";
			...
			break;
		case TRAP_DAM_ACID:
			messages.attacker       = "The blast of &+Gacid&n douses $N.";
			messages.victim         = "The blast of &+Gacid&n douses you.";
			messages.room           = "The blast of &+Gacid&n hits $N, who screams out in pain!";
			messages.death_attacker = "The blast of &+Gacid&n turns $N into a &+gsticky ooze&n.";
			messages.death_victim   = "Your body turns to &+gooze&n as the blast of &+Gacid&n hits you!";
			messages.death_room     = "The blast of &+Gacid&n turns $N into a &+gsticky puddle&n.";
			...
			break;
		case TRAP_DAM_ENERGY:
			messages.attacker       = "The &+cpulse&n of &+Yenergy&n shocks $N.";
			messages.victim         = "The &+cpulse&n of &+Yenergy&n shocks you.";
			messages.room           = "The &+cpulse&n of &+Yenergy&n shocks $N.";
			messages.death_attacker = "The &+cpulse&n of &+Yenergy&n turns $N to &+Lash&n.";
			messages.death_victim   = "The &+cpulse&n of &+Yenergy&n turns you to &+wash&n.";
			messages.death_room     = "The &+cpulse&n of &+Yenergy&n turns $N to a pile of &+Ycrackling&n &+wash&n.";
