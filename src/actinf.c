...
//            && !IS_SET(EXIT(ch, keyword_no)->exit_info, EX_CLOSED) )
  ...
  case 24:                     // looking 'inside' something - so we can show a room w/o the map
    ...
    if (IS_MAP_ROOM(room_no) && !IS_MAP_ROOM(ch->in_room) && cmd != CMD_LOOKOUT && !IS_SHIP_ROOM(ch->in_room)) map_look_room( ch, room_no, MAP_USE_TOGGLE );  // Fix looking from zone to map room -- Eikel.
