...
void spell_blood_alliance(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	act("Your forearm glows &+rred&n as you align it with $N's forearm.", FALSE, ch, 0, victim, TO_CHAR);
	act("Your forearm glows &+rred&n as $n aligns it with $s forearm.", FALSE, ch, 0, victim, TO_VICT);
...
void spell_ilienzes_flame_sword(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (ch->equipment[WIELD] && FLAME_REAVER_WEAPONS(ch->equipment[WIELD]))
	{
		weapon = true;
	}

	if (!weapon && ch->equipment[WIELD2] && FLAME_REAVER_WEAPONS(ch->equipment[WIELD2]))
	{
		weapon = true;
	}
...
bool ilienze_sword(P_char ch, P_char victim, P_obj wpn)
{
	...
	struct damage_messages messages = {"&+LYour&N $q &+Rburns&n $N &+Rwith a godly fire!&n",
	...
	struct damage_messages proc_messages = {ch_msg,
	                                        vict_msg,
	                                        room_msg,
	...
	if (IS_AFFECTED2(ch, AFF2_FIRESHIELD) && has_innate(ch, INNATE_IMMOLATE))
	{
...
{
	snprintf(messages->attacker, MAX_STRING_LENGTH, "&+rUnholy &+Rflames&n &+rflow down your blade and shoot in a %s &+rtowards $N&+r.&n", sub);
	snprintf(messages->victim, MAX_STRING_LENGTH, "&+rUnholy &+Rflames&n &+rflow down $n's &+rblade and shoot in a %s &+rtowards &+Ryou&+r.&n", sub);
	snprintf(messages->room, MAX_STRING_LENGTH, "&+rUnholy &+Rflames&n &+rflow down $n's &+rblade and shoot in a %s &+rtowards $N&+r.&n", sub);
}

void cegilunes_broken(struct char_obj_link_data *cold) { wear_off_message(cold->ch, cold->affect); }
...
void spell_cegilunes_searing_blade(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (!(((obj = ch->equipment[WIELD]) && FLAME_REAVER_WEAPONS(obj)) || ((obj = ch->equipment[WIELD2]) && FLAME_REAVER_WEAPONS(obj)) || ((obj = ch->equipment[WIELD3]) && FLAME_REAVER_WEAPONS(obj)) ||
...
bool cegilune_blade(P_char ch, P_char victim, P_obj wpn)
{
	...
	struct damage_messages tiers_messages[6] = {
		{																			 "&+LThe &+Rs&+re&+Ra&+rr&+Ri&+rn&+Rg &+Lblade slices into $N, causing $m to scream in pain!&n",
	     "&+L$n's $q &+Lglows &+rf&+Rie&+rry &+wh&+Wo&+wt &+Las it painfully brands your flesh.&n","&+L$N screams in &+rt&+Re&+rr&+Rr&+ro&+Rr &+Las $n's $q &+Lsears past $S armor into $S flesh!&n",
	     "&+LThe &+rs&+Rea&+rr&+Rin&+rg &+Lw&+round&+Ls are too much for $N to handle, and $E falls to the ground, thrashing wildly.&n","&+LAs $n's $q &+Lstrikes you, it opens up a fatal w&+roun&+Ld. The last thing you sense is the burning of your own &+rflesh&+L.&n",
	     "&+LThe &+rs&+Rea&+rr&+Rin&+rg &+Lw&+round&+Ls are too much for $N to handle, and $E falls to the ground, thrashing wildly.&n",DAMMSG_TERSE																																		},
		{																		  "&+LYour $q &+Ltakes on a mind of it's own as it &+rb&+Ru&+rr&+Rn&+rs&+L deep into $N's flesh.&n",
	     "&+w$n's $q &+Lglows &+rf&+Rie&+rry &+wh&+Wo&+wt &+was it painfully brands your flesh.&n",	       "&+L$N screams in &+rbl&+Roo&+rd&+wc&+rur&+Rdl&+rin&+Rg &+Lpain as $n's $q &+Lsears deep into $S flesh.&n",
	     "&+LThe &+rs&+Rea&+rr&+Rin&+rg &+Lw&+round&+Ls are too much for $N to handle, and $E falls to the ground, thrashing wildly.&n",    "&+LAs $n's $q strikes you, it opens up a fatal w&+roun&+Ld. The last thing you sense is the burning of your own &+rflesh&+L.&n",
	     "&+LThe &+rs&+Rea&+rr&+Rin&+rg &+Lw&+round&+Ls are too much for $N to handle, and $E falls to the ground, thrashing wildly.&n", DAMMSG_TERSE},
		{																																					"&+LYour $q &+Lbursts into &+rhe&+Rll&+ris&+Lh &+rf&+Rl&+ra&+Rm&+re&+Rs &+Land causes $N to scream in great and &+wa&+Lg&+wo&+Ln&+wi&+Lz&+wi&+Ln&+wg &+Lp&+rai&+Ln as it burns away at $S "
	                                                                                                                                                "&+rflesh&+L!", "&+rHe&+Rll&+Lis&+rh &+rf&+Rl&+ra&+Rm&+re&+Rs &+Lburst forth from $n's $q&+L, rending your flesh and causing you incredible p&+rai&+Ln!",
	     "&+rF&+Rl&+ra&+Rm&+re&+Rs &+Lburst forth from $q&+L, dancing along $N's &+rflesh&+L as $E screams in sheer &+Lp&+rai&+Ln!", "&+LThe &+rs&+Rea&+rr&+Rin&+rg &+Lw&+round&+Ls are too much for $N to handle, and $E falls to the ground, thrashing wildly.&n",
	     "&+LAs $n's $q strikes you, it opens up a fatal w&+roun&+Ld. The last thing you sense is the burning of your own &+rflesh&+L.&n", "&+LThe &+rs&+Rea&+rr&+Rin&+rg &+Lw&+round&+Ls are too much for $N to handle, and $E falls to the ground, thrashing wildly.&n",
	     DAMMSG_TERSE																																}
    };
	...
	if (!number(0, 4))
	{
		...
		{
			case 0:
				if (spell_damage(ch, victim, number(8, 16), SPLDAM_FIRE, SPLDAM_NODEFLECT, &tiers_messages[afp->modifier]) != DAM_NONEDEAD)
				{
					afp->modifier = 0;
					return TRUE;
				}
				break;
			case 1:
				if (spell_damage(ch, victim, number(16, 32), SPLDAM_FIRE, SPLDAM_NODEFLECT, &tiers_messages[afp->modifier]) != DAM_NONEDEAD)
				{
					afp->modifier = 0;
					return TRUE;
				}
				break;
			case 2:
				if (spell_damage(ch, victim, number(32, 40), SPLDAM_FIRE, SPLDAM_NODEFLECT, &tiers_messages[afp->modifier]) != DAM_NONEDEAD)
				{
					afp->modifier = 0;
					return TRUE;
				}
				break;
			case 3:
				if (spell_damage(ch, victim, number(40, 56), SPLDAM_FIRE, SPLDAM_NODEFLECT, &tiers_messages[afp->modifier]) != DAM_NONEDEAD)
				{
					afp->modifier = 0;
					return TRUE;
				}
				break;
			case 4:
				if (spell_damage(ch, victim, number(56, 64), SPLDAM_FIRE, SPLDAM_NODEFLECT, &tiers_messages[afp->modifier]) != DAM_NONEDEAD)
				{
					afp->modifier = 0;
					return TRUE;
				}
				break;
			default:
				if (spell_damage(ch, victim, number(64, 80), SPLDAM_FIRE, SPLDAM_NODEFLECT, &tiers_messages[5]) != DAM_NONEDEAD)
				{
					afp->modifier = 0;
					return TRUE;
				}
	...
	if (afp->modifier >= 3)
	{
		if (!number(0, 5))
		{
			act("&+LYour $q&+L suddenly ig&+rni&+Rte&+rs &+Linto &+rf&+Rl&+ra&+Rm&+re&+Rs &+Las it strikes $N!&n", FALSE, ch, wpn, victim, TO_CHAR);
			act("&+L$n's $q&+L ignites into a raging &+ri&+Rn&+rf&+Re&+rr&+Rn&+ro &+Las it bites deep into your &+rflesh&+L.&n", FALSE, ch, wpn, victim, TO_VICT);
			act("&+LRaging &+rhe&+Rll&+rfi&+Lre bursts forth from $n's $q&+L as it slices into $N!&n", FALSE, ch, wpn, victim, TO_NOTVICT);
...
void event_cegilune_searing(P_char ch, P_char vict, P_obj wpn, void *data)
{
	struct damage_messages messages = {
		...
		"",
		"&+L$N &+Lsuddenly begins thrashing around violently. Several chunks of smoldering &+rflesh&+L fall from $S body, and $E topples to the ground, screaming violently--then is suddenly "
		"silent!&n",
		"&+LThe &+rf&+Ri&+re&+Rr&+ry &+Lp&+rai&+Ln is too much for you to handle, and you think you hear the incessant cackling of &+md&+Lemoni&+mc &+Llaughter as your vision spins toward "
		"oblivion...&n",
...
bool girilals_granite_hammer(P_char ch, P_char victim, P_obj wpn)
{
	...
	struct damage_messages messages = {"&+LThe &+ydust&+L surrounding $p &+Lscratches at $N&+L!&n",
...
void ileshs_broken(struct char_obj_link_data *cold) { wear_off_message(cold->ch, cold->affect); }
...
void spell_ileshs_smashing_fury(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	// If they don't have a prime or secondary reaver weapon.
	if (!(((weapon = ch->equipment[WIELD]) && EARTH_REAVER_WEAPONS(weapon)) || ((weapon = ch->equipment[WIELD2]) && EARTH_REAVER_WEAPONS(weapon)) ||
	      ((weapon = ch->equipment[WIELD3]) && EARTH_REAVER_WEAPONS(weapon)) || ((weapon = ch->equipment[WIELD4]) && EARTH_REAVER_WEAPONS(weapon))))
	{
		send_to_char("You need to wield the correct type of weapon!\n", ch);
		return;
	}
...
void spell_thryms_icerazor(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (ch->equipment[WIELD] && FROST_REAVER_WEAPONS(ch->equipment[WIELD]))
	{
		weapon = true;
	}

	if (!weapon && ch->equipment[WIELD2] && FROST_REAVER_WEAPONS(ch->equipment[WIELD2]))
	{
		weapon = true;
	}
...
bool thryms_icerazor(P_char ch, P_char victim, P_obj wpn)
{
	...
	struct damage_messages messages = {"&+CThe shards of &+Bice&+C surrounding $q &+Csplinter off as they strike $N&+C!&n",
	...
	if (IS_AFFECTED3(ch, AFF3_COLDSHIELD) && number(1, 100) <= 5)
	{
...
void spell_lliendils_stormshock(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (ch->equipment[WIELD] && SHOCK_REAVER_WEAPONS(ch->equipment[WIELD]))
	{
		weapon = true;
	}

	if (!weapon && ch->equipment[WIELD2] && SHOCK_REAVER_WEAPONS(ch->equipment[WIELD2]))
	{
		weapon = true;
	}
...
bool lliendils_stormshock(P_char ch, P_char victim, P_obj wpn)
{
	...
	struct damage_messages messages = {"&+BSearing &+Yb&+Colt&+Ys &+Bof &+Lblack &+Blig&+Whtn&+Bing &+Bflow forth from your $q, &+Bstriking $N!&n",
	...
	if (IS_AFFECTED3(ch, AFF3_LIGHTNINGSHIELD) && number(1, 100) <= 5)
	{
...
bool kostchtchies_implosion(P_char ch, P_char victim, P_obj wpn)
{
	...
	else
	{
		...
		if (!number(0, 1))
		{
			...
			{
				afp->modifier                   = 0;
				struct damage_messages messages = {"&+CSuddenly, a the &+Lshards &+Cof &+Bice &+Cfly forth from your weapon&+C, slicing into $N's &+Cflesh!&n",
...
void apply_reaver_mods(P_char ch)
{
	...
	{
		if ((w1 && required_weapon_skill(w1) == SKILL_1H_BLUDGEON) || (w2 && required_weapon_skill(w2) == SKILL_1H_BLUDGEON))
		{
			ch->specials.base_combat_round += get_property("reavers.ice.penalty.pulse.1hBludgeon", 2.);
		}
		else if ((w1 && required_weapon_skill(w1) == SKILL_2H_BLUDGEON) || (w2 && required_weapon_skill(w2) == SKILL_2H_BLUDGEON))
		{
	...
	{
		if (required_weapon_skill(w1) == SKILL_2H_SLASHING || required_weapon_skill(w2) == SKILL_2H_SLASHING || required_weapon_skill(w1) == SKILL_2H_FLAYING ||
